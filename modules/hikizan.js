/////問題エリアの生成/////////////
const classHikizans = ["left", "plus", "right", "equal", "anser"];
const mainContainerHikizan = document.querySelector("#main-containerHikizan");
document.getElementById("count-hikizan").textContent =
  "100てん めざして がんばろう";
/////div生成、クラス付与
classHikizans.forEach((classHikizan) => {
  const a = document.createElement("div");
  a.classList.add(classHikizan);
  mainContainerHikizan.appendChild(a);
});

/////ランダム配列を生成して問題を表示する関数////////
export const makeRandomArry = () => {
  /////ランダムの数字を生成(arry2に格納)///
  let arry1 = [];
  let arry2 = [];
  /////配列の範囲を設定
  ...
  document.querySelector(".left").textContent = arry2[0];
  document.querySelector(".right").textContent = arry2[6];
  document.querySelector(".anser").textContent = arry2[0] + arry2[6];
};
makeRandomArry();
